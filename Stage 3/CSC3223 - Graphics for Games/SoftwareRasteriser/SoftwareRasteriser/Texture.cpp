...
Colour Texture::BilinearTexSample(const Vector3 &coords, int miplevel) {
	...
	const Colour &tl = ColourAtPoint(x, y, miplevel);			//Our tex point
	const Colour &tr = ColourAtPoint(x + 1, y, miplevel);		//One to Right
	const Colour &bl = ColourAtPoint(x, y+1, miplevel);			//One Below
	const Colour &br = ColourAtPoint(x + 1, y + 1, miplevel);	//Below Right
...
void Texture::CreateMipMaps() {
	...
	while (tempWidth > 1 && tempHeight > 1) {
		tempWidth = tempWidth >> 1; //Bitshifting halves value
...
void Texture::GenerateMipLevel(Colour* source, Colour* dest, int level) {
	...
	int outY = 0;	//increments at half rate of for loop

	for (int y = 0; y < sourceHeight; y += 2) {
		int outX = 0;	//Increments at half rate of for loop 
